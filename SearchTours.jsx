...
function parsePDFText(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // ── Détection barcode ──
    // On ne cherche les barcodes QUE sur les lignes qui contiennent LV1_, LV2_, LV3_
    // C'est la seule ligne où le barcode apparaît de façon fiable dans ce PDF
    if (!line.match(/LV[123]_/)) continue

    // Le barcode est TOUJOURS le dernier groupe de chiffres sur cette ligne
    ...
    while ((m = barcodeRegex.exec(line)) !== null) {
      ...
      if (bc.match(/^336\d|^337\d/)) continue                    // tel sans indicatif 336x/337x
      if (bc.match(/^33[67]/)) continue                          // tel +33 colle (ex: 330658421692)
